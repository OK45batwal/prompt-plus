          ...
          {showOtp ? (
            <>
              <div className="text-center mb-8">
                <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-4">
                  {verified ? <Check className="h-6 w-6 text-green-600" /> : <Mail className="h-6 w-6 text-primary" />}
                </div>
              ...
              {otpError && (
                <div className="mb-4 p-3 rounded-lg text-xs text-center" style={{ color: otpError === "New code sent!" ? 'var(--green-600)' : 'var(--red-600)' }}>
                  {otpError}
                </div>
              )}
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-3">
                ...
                >
                  {otpLoading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Verifying...</> : <>Verify email</>}
                </button>
              </form>

              <p className="text-center text-sm text-muted-foreground mt-4">
                Didn&apos;t get it?{" "}
                <button onClick={handleResendOtp} className="text-foreground hover:underline cursor-pointer bg-transparent border-none p-0 text-sm">
                  Resend code
                </button>
              </p>
            </>
            ...
            <>
              <div className="text-center mb-8">
                ...
                <p className="text-sm text-muted-foreground mt-1">Start improving your prompts for free</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <div className="relative">
                  ...
                  <Input type="text" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} required className="pl-9" />
                </div>
                <div className="relative">
                ...
                <div className="relative">
                  ...
                  <Input type={showPassword ? "text" : "password"} placeholder="Password (8+ characters)" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} className="pl-9 pr-9" />
                ...
                <button type="submit" disabled={isLoading}
                  className="h-9 w-full inline-flex items-center justify-center rounded-lg bg-foreground text-background px-4 text-sm font-medium hover:bg-foreground/90 transition-colors disabled:opacity-50">
                  {isLoading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Creating account...</> : <>Sign up <ArrowRight className="h-4 w-4 ml-2" /></>}
                </button>
              </form>
